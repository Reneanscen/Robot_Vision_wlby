...
geometry_msgs::msg::Point getCentroid(const std::vector<geometry_msgs::msg::Point> points)
{
...
bool computeCorrespondences(const std::vector<geometry_msgs::msg::Point>& source,
              ...
              geometry_msgs::msg::Transform& t)
{
  ...
  // Get centroid of source and target 计算质心
  ...
  // Compute P for source ;;;;   source point 归一化
  ...
  Eigen::MatrixXf A = solver_s.eigenvectors().real(); //特征向量
  Eigen::MatrixXf eig_of_Ms = solver_s.eigenvalues().real();  //特征值
  
  //std::cout << A << std::endl;
  //std::cout << eig_of_Ms << std::endl;
