    ...
    if (!Exist(idx)) {
      ...
      for (int obs_idx = head_[idx]; obs_idx != undefined_; obs_idx = next_obs_idx) {
        ...
        for (const auto &dir : dirs_) {
          ...
          if (VoxInRange(new_pos) && closest_obstacle_[idx] != Eigen::Vector3i(undefined_, undefined_, undefined_)
  ...
  while (!update_queue_.empty()) {
    ...
    for (int i = 0; i < num_dirs_; i++) {
      ...
      if (VoxInRange(new_pos)) {
        int new_pos_idx = Vox2Idx(new_pos);
        if (closest_obstacle_[idx] == Eigen::Vector3i(undefined_, undefined_, undefined_))
          continue;
...
void fiesta::ESDFMap::GetPointCloud2(sensor_msgs::msg::PointCloud2 &m, int vis_lower_bound, int vis_upper_bound) {
    m.header.frame_id = "world";
    m.header.stamp = rclcpp::Clock().now();  // 确保时间戳被设置
    ...
    m.width = points.size();
    m.row_step = m.width * sizeof(float) * 3;
    m.point_step = sizeof(float) * 3;
...
bool fiesta::ESDFMap::CheckConsistency() {
  ...
  for (int ii = 1; ii < count; ii++) {
    ...
    if (prev_[vox] != undefined_ && next_[prev_[vox]] != vox
      ...
      std::cout << vox_buffer_  << ' ' << vox_buffer_  << ' ' << vox_buffer_  << ' ' << vox
                << std::endl;
...
bool fiesta::ESDFMap::CheckWithGroundTruth() {
  ...
  for (int ii = 1; ii < count; ii++)
    if (Exist(ii)) {
      ...
      cloud_->points ;
      tot_++;
      if (vox_buffer_  > ma(0)) ma(0) = vox_buffer_ ;
      if (vox_buffer_  > ma(1)) ma(1) = vox_buffer_ ;
      if (vox_buffer_  > ma(2)) ma(2) = vox_buffer_ ;
      if (vox_buffer_  < mi(0)) mi(0) = vox_buffer_ ;
      if (vox_buffer_  < mi(1)) mi(1) = vox_buffer_ ;
      if (vox_buffer_  < mi(2)) mi(2) = vox_buffer_ ;
  ...
  for (int ii = 1; ii < count; ii++) {
    if (distance_buffer_[ii] >= 0 && distance_buffer_[ii] < infinity_) {
      kdtree.nearestKSearch(pcl::PointXYZ(vox_buffer_ , vox_buffer_ , vox_buffer_ ), 1,
