...
from tasks.transparent.corelibs.utils.gpfit import gaussian, moments, fitgaussian
...
def object_pose_decode(
    ...
    kps[..., ::2] += xs.view(batch, K, 1).expand(batch, K, num_joints)  # + centroid loc x
    kps[..., 1::2] += ys.view(batch, K, 1).expand(batch, K, num_joints)    # + centroid loc y
    if reg is not None:               # 为预测的中心点加偏移（弥补下采样过程中精度损失）
    ...
    if wh is not None:          # 通过中心点和预测的w 和 h 计算bbox 的 xmin ymin xmax ymax   # noqa
        ...
        if hm_hp is not None:
            ...
            hm_hp = _nms(hm_hp)         # 对8个顶点(channel=8)的heatmap作nms
            thresh = 0.1
            kps = kps.view(batch, K, num_joints, 2).permute(
                0, 2, 1, 3).contiguous()  # b x J x K x 2

            # mask_temp = torch.ones((batch, num_joints, K, 2)).to(kps.device)
            kps_displacement_mean = kps
            ...
            dist = (((reg_kps - hm_kps) ** 2).sum(dim=4) ** 0.5)  # b x J x K x K       # 这里相当于一个匹配的过程，两两进行计算距离
            min_dist, min_ind = dist.min(dim=3)  # b x J x K                            # 为每一个heatmap的点找一个最佳的 displacement point(也就是由中心点加上偏移得到的2d点)
            hm_score = hm_score.gather(2, min_ind).unsqueeze(-1)  # b x J x K x 1       # 通过这个匹配关系取出每个
            min_dist = min_dist.unsqueeze(-1)
            min_ind = min_ind.view(batch, num_joints, K, 1, 1).expand(
                batch, num_joints, K, 1, 2)
            hm_kps = hm_kps.gather(3, min_ind)                                          # 通过index去获取对应的point
            ...
            if rep_mode == 3:
                ...
                kps = (1 - mask) * hm_kps + mask * kps                                  # 两种检测出来的点通过mask进行融合
