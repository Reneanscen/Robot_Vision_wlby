...
from tasks.transparent.corelibs.models.model import create_model, load_model
from tasks.transparent.corelibs.utils.image import get_affine_transform, affine_transform
from tasks.transparent.corelibs.utils.debugger import Debugger

from tasks.transparent.corelibs.utils.pnp.cuboid_pnp_shell import pnp_shell

from tasks.transparent.corelibs.utils.tracker import Tracker
from tasks.transparent.corelibs.utils.tracker_baseline import Tracker_baseline

from tasks.transparent.corelibs.utils.image import draw_umich_gaussian, gaussian_radius, draw_nvidia_gaussian
...
class BaseDetector(object):
    ...
    def pre_process(self, image, scale, input_meta={}):
        ...
        if 'pre_dets' in input_meta.keys():
            meta['pre_dets'] = input_meta['pre_dets']
        if 'camera_matrix' in input_meta.keys():
            meta['camera_matrix'] = input_meta['camera_matrix']
        if 'id' in input_meta.keys():
            ...
            for bbox in results:
                ...
                if self.opt.rep_mode == 0 or self.opt.rep_mode == 3 or self.opt.rep_mode == 4:
                    ...
                    for i in range(8):
                        ...
                        clf = mixture.GaussianMixture(n_components=2, covariance_type='full')           # x, y 方向两个gauss
                        clf.fit(X_train)
                        GMM_list.append(clf)

                        points_sample = clf.sample(N_sample)                                            # 从混合后的分布里抽样一个点
                ...
                ret = pnp_shell(self.opt, meta, bbox, points_filtered, bbox['obj_scale'], OPENCV_RETURN=True)        #self.opt.show_axes)
                if ret is not None:
                    boxes.append(ret)
