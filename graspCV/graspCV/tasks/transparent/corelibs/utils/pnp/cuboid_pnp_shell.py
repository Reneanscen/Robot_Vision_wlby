...
def pnp_shell(opt, meta, bbox, points_filtered, scale, OPENCV_RETURN = True):
    cuboid3d = Cuboid3d(1 * np.array(scale) / scale[1])     # 初始化原始的3D包围盒         glass cup test size = 0.085

    pnp_solver = CuboidPNPSolver(opt.c, cuboid3d=cuboid3d)          #
    ...
    if location is not None:
        ...
        bbox['point_3d_obj'] = point_3d_obj
        ...
        point_3d_cam_test = meta['camera_matrix'] @ point_3d_cam.T
        test = point_3d_cam_test[0:2, :] / point_3d_cam_test[2:, :]
        print(location)
        # print("test1: ", test.T)
        # print("test2: ", projected_points)
